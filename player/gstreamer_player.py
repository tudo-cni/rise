...
class StreamReceiver:
    ...
    def create_pipeline(self):
        ...
        caps = Gst.Caps.from_string(f"application/x-rtp,media=video,clock-rate=90000,encoding-name=H{mode},payload=96")
        ...
        self.depay = Gst.ElementFactory.make(f"rtph{mode}depay", "depay")
        parser = Gst.ElementFactory.make(f"h{mode}parse", "parser")
        timestamper = Gst.ElementFactory.make(f"h{mode}timestamper","timestamper") #generate missing pts

        framerate_filter = Gst.ElementFactory.make("capsfilter", "framerate-filter")
        framerate_caps = Gst.Caps.from_string(f"video/x-h{mode},framerate=30/1")  # Set to 60 FPS
        framerate_filter.set_property("caps", framerate_caps)

        # Use a video sink to display the video
        video_decoder = Gst.ElementFactory.make(f"avdec_h{mode}", "video-decoder")  # Decode Hxxx stream
        videoconvert = Gst.ElementFactory.make("videoconvert", "videoconvert")
        
        video_sink = Gst.ElementFactory.make("autovideosink", "video-sink")     # Display video

        # Add elements to the pipeline
        elements = [udpsrc, capsfilter, jitterbuffer, self.depay, parser, timestamper, framerate_filter, video_decoder, videoconvert, video_sink]
...
if __name__ == "__main__":
    ...
    logging.info("Starting Adaptive Video Stream Player")
